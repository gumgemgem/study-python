...
i 表示由外至内第 i+1 层数组中进行计算，然后按照由外至内第 i+1 层数组本来的方式进行组合  
总结，i 表示函数沿着第 i 个下标变化的方式进行计算，[详见](https://zhuanlan.zhihu.com/p/31275071)  
...
print(np.max(three_array, axis=0))
print('---------------------')
print(np.max(three_array, axis=1))
print('---------------------')
print(np.max(three_array, axis=2))
